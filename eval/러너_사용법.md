...
guidelines/8_금칙어.md   # 유일한 정본, 별도 복사본을 만들지 않는다
```
...
429가 오면 `Retry-After` 또는 지수 backoff로 재시도하고, 재시도를 소진하면 실패 row를 남긴 뒤 다음 항목으로 간다.
...
HTTP 429 재시도까지 실패해도 같은 방식으로 남기고 다음 입력을 계속 실행한다.
...
└─ eval/
   ...
   ├─ forbidden.py        # guidelines/8_금칙어.md를 직접 파싱
   ├─ providers/
   └─ raw/
```
...
- 실제 결과는 기존 `results.md` 아래에 누적하며, 동일 섹션은 중복 기록하지 않는다.
