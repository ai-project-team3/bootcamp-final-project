...
# 이 문턱 아래면 그 칸을 비운다. 벤더 권고는 0.9 자동 / 0.5~0.9 확인 / 0.5 미만 사람이고,
# 규칙 7이 "놓치는 것보다 과차단이 위험하다"이므로 **낮게 잡아 통과시키는** 쪽으로 둔다.
# ⚠️ 이 값은 아직 근거가 없다. 1번 측정에서 확신도별 실제 정답률을 보고 고친다.
CONFIDENCE_FLOOR = float(os.getenv("TYPESAFE_CONFIDENCE_FLOOR", "0.0"))
