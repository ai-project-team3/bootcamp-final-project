...
### ⚠️ 지금 픽스처는 앱보다 3문항 뒤처져 있다 (2026-09-21)

앱의 `d_role_do` · `d_need_why` 질문에서 **뒤에 붙어 있던 `"왜?"` 를 뗐다.** 의문사가 둘이라
`guidelines/7_프롬프트.md` §1(한 번에 한 가지만 묻는다)에 어긋났다. `StoryBank.kt` 두 줄이다.

**픽스처는 일부러 안 다시 뽑았다.** `results.md` 의 모델 비교가 이미 현재 100문항으로 측정됐다.
지금 다시 뽑으면 **측정한 결과와 평가셋이 어긋난다** — 「모델마다 프롬프트를 손보지 않는다」와 같은 이유다.

| | |
| --- | --- |
| 어긋난 문항 | `j041` · `j058` · `j067` — `context` 가 앱의 현재 질문과 다르다 |
| 라벨 | **그대로 맞다.** 아이가 실제로 답한 것만 라벨하므로 질문이 둘을 물었어도 판정은 같다 |
| 언제 맞추나 | **9/23 발표 뒤.** 위 ①②③ 절차를 그대로 돌리면 97개는 자동 복원되고 이 3개만 목록으로 나온다 |
