...
def main() -> None:
    ...
    with io.open(out, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for n, it in enumerate(items(a.limit), 1):
            r = w(it["wav"], None)
            v = check_transcript(r["hyp"])
            row = {"clip_id": it["clip_id"], "engine": "whisper_turbo", "condition": it["condition"], "is_silence": "0",
    ...
    by = {}
    for r in rows:
        g = by.setdefault(r["condition"], {"n": 0, "err": 0, "len": 0, "wok": 0, "w": 0, "lat": [], "drop": 0, "exact": 0})
    ...
    print("| 조건 | 클립 | CER | 문장 정확 일치 | 어절 보존 | 헛문장으로 버림 | 지연 p50 | p95 |")
    print("|---|---:|---:|---:|---:|---:|---:|---:|")
    for c, g in sorted(by.items()):
        print(f"| {c} | {g['n']} | {g['err']/g['len']*100:.1f}% | {g['exact']/g['n']*100:.1f}% | {g['wok']/g['w']*100:.1f}% | {g['drop']} | "
    ...
    for r in worst:
        print(f"- [{r['condition'][:5]}] 「{r['ref_heard']}」 → 「{r['hyp']}」")
    drops = [r for r in rows if r["dropped"]]
    if drops:
        print("\n⚠️ 헛문장으로 버린 아이 말:")
        for r in drops:
            print(f"- 「{r['ref_heard']}」 → 「{r['hyp']}」 ({r['dropped']})")
